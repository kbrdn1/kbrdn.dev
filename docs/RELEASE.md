...
| Gate de vérif | `bun run lint` + `bun run fmt:check` + `bun run typecheck` + `bun run build` + `scripts/og-smoke.sh` (CI, tous bloquants). **Pas de tests** dans ce repo, hors `scripts/*.test.sh` et ce smoke test des images OG |
