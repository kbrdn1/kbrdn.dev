...
# sur deux lignes (/_og/* en 500), puis `'Inter'` en guillemets simples dans le
# CSS (200, mais police de repli). Le statut attrape le premier, seule la
# comparaison à une référence attrape le second.
#
...
curl -sf -o /dev/null "$BASE/api/health" || { echo "le serveur ne répond pas" >&2; cat "$OUT/server.log" >&2; exit 1; }
...
for entry in "${PAGES[@]}"; do
  ...
  url=$(curl -s "$BASE$page" | sed -n 's/.*property="og:image" content="\([^"]*\)".*/\1/p' | head -1 | sed 's/&amp;/\&/g')
  if [[ -z "$url" ]]; then
    echo "  FAIL $page : pas d'og:image dans la page"
    FAILED=1
    continue
  fi
  rel="/${url#*://*/}"
  got="$OUT/$name.png"
  meta=$(curl -s -o "$got" -w '%{http_code} %{content_type}' "$BASE$rel")
